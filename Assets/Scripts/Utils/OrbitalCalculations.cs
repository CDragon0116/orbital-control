...
public static class OrbitalCalculations
{
    ...
    public static OrbitalParameters CalculateOrbitalParameters(float centralBodyMass, Vector3 centralBodyPosition, Transform bodyTransform, Vector3 velocity)
    {
        ...
        // ν = acos( (e · r) / (|e||r|) ), sign determined by r·v
        if (eVec.magnitude > 1e-6f && r.magnitude > 1e-6f)
        {
            float dotEr = Mathf.Clamp(Vector3.Dot(eVec, r) / (eVec.magnitude * r.magnitude), -1f, 1f);
            float nu = Mathf.Acos(dotEr) * Mathf.Rad2Deg;
            if (Vector3.Dot(r, v) < 0f)
                nu = 360f - nu;
            result.trueAnomaly = nu;
        }
        else
        {
        ...
        // Argument of perigee (ω) - angle from ascending node to eccentricity vector, in orbital plane
        // Compute using node vector and eccentricity vector (eVec computed earlier)
        // If orbit is near-circular or node undefined, set to 0.
        if (result.eccentricity > 1e-6f && nodeMag > 1e-6f)
        {
            Vector3 nodeUnit = nodeVec.normalized;
            eUnit = eVec.magnitude > 1e-6f ? eVec.normalized : Vector3.zero;

            float dotNe = Mathf.Clamp(Vector3.Dot(nodeUnit, eUnit), -1f, 1f);
            float arg = Mathf.Acos(dotNe) * Mathf.Rad2Deg;

            // Determine correct orientation: if (node x e) · h < 0 then take 360 - arg
            Vector3 cross = Vector3.Cross(nodeUnit, eUnit);
            if (Vector3.Dot(cross, hVec) < 0f)
                arg = 360f - arg;

            result.argumentOfPerigee = arg;
        }
        else
        {
