...
public class UIManager : MonoBehaviour
{
    ...
    public SatelliteListPanel satelliteListPanel;
    public Button viewSatellitesButton;
 ...
 "切换到自由视角以自由探索或放置卫星";
        ...
        UpdateButtonText();
        deltaVText.text = "";
if (viewSatellitesButton != null)
{
    viewSatellitesButton.gameObject.SetActive(true); // 确保它是可见的
    SetButtonState(viewSatellitesButton, false); // <-- 添加这一行, 设置为 "未按下" 颜色
}
        if (satelliteListPanel != null)
            satelliteListPanel.Initialize(ctx);
    }
...
"- 点击 '放置卫星' 以生成";
    ...
    // --- 添加/修改这些行 ---
    if (viewSatellitesButton != null)
        viewSatellitesButton.gameObject.SetActive(false); // 隐藏 "查看卫星" 按钮

    if (satelliteListPanel != null && satelliteListPanel.gameObject.activeSelf)
    {
        satelliteListPanel.Hide(); 
        // SetButtonState(viewSatellitesButton, false); // 确保状态重置 (虽然它隐藏了)
    }
    // --- 添加/修改结束 ---
    }
...
"切换到自由视角以自由探索或放置卫星";
    ...
    if (viewSatellitesButton != null)
    {
        ...
        if (satelliteListPanel == null || !satelliteListPanel.gameObject.activeSelf)
        {
            viewSatellitesButton.gameObject.SetActive(true);
            SetButtonState(viewSatellitesButton, false); // 确保它是 "未按下" 颜色
        }
    }
    // --- 添加/修改结束 ---
    }
  ...
  public void OpenSatelliteList()
{
    if (satelliteListPanel != null)
        satelliteListPanel.Show(); // 调用 SatelliteListPanel 脚本中的 Show()

    if (viewSatellitesButton != null)
        SetButtonState(viewSatellitesButton, true); // <-- 修改这一行, 设为 "按下" 颜色

    EventSystem.current.SetSelectedGameObject(null);
}

/// <summary>
/// 由 SatelliteListPanel 内部的 "CloseButton" 调用。
/// 关闭卫星列表面板，并重新显示 "查看卫星" 按钮。
/// </summary>
public void CloseSatelliteList()
{
    if (satelliteListPanel != null)
        satelliteListPanel.Hide(); // 调用 SatelliteListPanel 脚本中的 Hide()

    if (viewSatellitesButton != null)
       SetButtonState(viewSatellitesButton, false); // <-- 修改这一行, 设为 "未按下" 颜色
